...
export const LLMApiKeyComponent = (p: {
  ...
  if (!hasAccess) {
    return (
      <div>
        <Label className="text-xs font-semibold">{t("llm-api-key.api-key-visible-label", "API key")}</Label>
        <p className="text-muted-foreground text-sm">
          {t("llm-api-key.api-key-restricted-desc", "LLM API Key only visible to Owner and Admin roles.")}
        </p>
      </div>
    );
  }
  ...
  if (apiKeys.isPending) {
    return (
      <div>
        <Label className="text-xs font-semibold">{t("llm-api-key.api-key-visible-label", "API key")}</Label>
        <p className="text-muted-foreground text-sm">{t("layout.loading", "Loading")}...</p>
      </div>
    );
  }
