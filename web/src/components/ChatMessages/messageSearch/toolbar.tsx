  ...
  if (!isOpen) {
    return (
      <Button
        ...
        onClick={openSearch}
        aria-label={t("chat.search.aria", "Find in messages")}
      >
  ...
  return (
    ...
    >
      ...
      <Input
        ...
        placeholder={t("chat.search.placeholder", "Find in messages")}
      ...
      <IconButton
        icon={ChevronUp}
        label={t("chat.search.previous", "Previous result")}
        onClick={previousMatch}
        disabled={matches.length === 0}
      />
      <IconButton
        icon={ChevronDown}
        label={t("chat.search.next", "Next result")}
        onClick={nextMatch}
        disabled={matches.length === 0}
      />
      <IconButton
        icon={X}
        label={t("chat.search.close", "Close search")}
        onClick={closeSearch}
      />
    </div>
  );
